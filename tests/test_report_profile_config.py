...
def test_normalize_site_env_config_report_page_margin_removed() -> None:
    cfg = normalize_site_env_config(
        {
            "report": {
    ...
    assert cfg["report"]["pdf_profile_id"] == "substation_daily_a4"
    assert "page_margin_mm" not in cfg["report"]
...
def test_resolve_pdf_render_plan_default_profile() -> None:
    ...
    assert "page_margin_mm" not in plan
